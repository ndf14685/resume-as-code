...
def render_analysis_md(
    ...
    *,
    ...
    """`matrix` is the JD↔evidence MatchMatrix. When supplied, the covered /
    transferable / gap sections come from the parsed JD instead of from
    EXTERNAL_TERMS — a ~30-entry lexicon curated for past vacancies, which is
    why a JD full of Veeam, Azure Landing Zones and Entra ID used to render
    'No hard gaps detected'."""
    if matrix is not None:
        return _render_matrix_md(matrix, job_name=job_name,
...
def _render_matrix_md(matrix, *, job_name: str, profile_name: str,
                      canonical_hash: str) -> str:
    """Job-match report driven by the real requirements matrix."""
    lines: list[str] = [
        f"# Job Match Analysis — {job_name}",
        "",
        f"- **Positioning:** `{profile_name}`",
        f"- **Canonical fingerprint:** `{canonical_hash[:16]}…`",
        f"- **Weighted must-have coverage:** "
        f"{round(matrix.coverage(musts_only=True) * 100)}%",
        "- **Method:** the JD is parsed into weighted requirements, each matched "
        "against the candidate's granular evidence inventory. No experience, "
        "skill or metric is invented.",
        "",
        "## 1. Requirements covered (real evidence)",
        "",
        "| Requirement | Priority | Evidence | Experiences |",
        "| --- | --- | --- | --- |",
    ]
    claimable = sorted(matrix.claimable(), key=lambda m: -m.requirement.weight)
    if claimable:
        for m in claimable:
            ev = ", ".join(e for e in m.evidence if not e.startswith("text:")) \
                or "verbatim in canonical experience text"
            lines.append(f"| {m.term} | {m.requirement.priority} | {ev} | "
                         f"{', '.join(m.experience_ids) or '—'} |")
    else:
        lines.append("| _none_ | | | |")

    lines += ["", "## 2. Partial / adjacent evidence (NOT claimed)", "",
              "These JD tools are **not** asserted. Related evidence of the same "
              "purpose exists and is listed for a human to judge.", ""]
    partials = matrix.partials()
    if partials:
        lines += ["| JD term (not claimed) | Adjacent evidence |", "| --- | --- |"]
        for m in partials:
            lines.append(f"| {m.term} | {', '.join(m.evidence)} |")
    else:
        lines.append("_None._")

    lines += ["", "## 3. Gaps (required, no evidence — NOT claimed)", ""]
    gaps = sorted(matrix.gaps(), key=lambda m: (-m.requirement.weight, m.term))
    if gaps:
        for m in gaps:
            lines.append(
                f"- **{m.term}** ({m.requirement.dimension}, "
                f"{m.requirement.priority}) — no evidence in the candidate "
                "knowledge base; deliberately omitted.")
    else:
        lines.append("_No gaps: every parsed requirement has real evidence._")

    lines += ["", "## 4. Terms incorporated into the CV", "",
              ", ".join(sorted({e for m in claimable for e in m.evidence
                                if not e.startswith("text:")})) or "_None._",
              "", "---",
              "_Generated by resume-as-code. Facts are protected by the canonical "
              "fingerprint; this report reflects selection, not invention._", ""]
    return "\n".join(lines)
